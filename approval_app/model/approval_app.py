...
class ApprovalApp(models.Model):
    ...
    industry=fields.Many2one('industry.company',related='contact.industry_cus')
    sub_sector=fields.Many2one('sub.sector',related='contact.sub_sector_cus')
    ...
    stages=fields.Selection([
        ...
        ('osd_officer_date_approved', 'Approved by OSD'),
        ('send_lead_group', 'Sent to lead group'),
        ('date_approved_lead', 'Lead Group Approved'),
        ('sent_council', 'Sent to Council'),
        ('returned_from_council', 'Date returned from Council'),
    ...
    def action_date_approved_by_ocd_officer(self):
        self.write({
            'stages': 'osd_officer_date_approved',
            'date_approved_by_ocd_officer': fields.datetime.now()
        })

    def action_date_appro_by_lead_group(self):
        self.write({
            'stages': 'date_approved_lead',
            'date_appro_by_lead_group': fields.datetime.now()
        })

    def action_date_return_from_council(self):
        self.write({
            'stages': 'returned_from_council',
            'date_return_from_council': fields.datetime.now()
        })
    ...
    def action_admin_approval_submission(self):
        ...
        self.write({
            'stages': 'admin_officer',


        })
    def action_admin_approval_date(self):
        self.write({
    ...
    def action_osd_officer_submission(self):
        ...
        self.write({
            ...
            'date_osd_officer_assigned': fields.datetime.now()
        })
    ...
    def _onchange_contact(self):
        if self.contact:
            ...
            self.cell_phone = self.contact.mobile
...
class TaskDescription(models.Model):
    ...
    detail_task=fields.Many2one('task.details','Details Of Task')
...
class ResPartner(models.Model):
    ...
    industry_cus=fields.Many2one('industry.company')
    sub_sector_cus=fields.Many2one('sub.sector')
